...
def parse_pdf(data):
    ...
    with pdfplumber.open(io.BytesIO(data)) as pdf:
        ...
        for p, page in enumerate(pdf.pages, 1):
            ...
            if "Entrada" not in df or "Sortida" not in df:
                continue
            for col in df.columns[1:]:
                ...
                if (
                    s.size
                    and e.size
                    and re.fullmatch(r"\d{1,2}:\d{2}", s[0])
                    and re.fullmatch(r"\d{1,2}:\d{2}", e[0])
                ):
...
st.write(f"Current UTC Time: {utc_now}")
st.write(f"Current Local Time (Madrid): {local_now}")
