import io, re, datetime as dt, streamlit as st, pdfplumber, pandas as pd
from google_auth_oauthlib.flow import Flow
from google.oauth2.credentials import Credentials
from googleapiclient.discovery import build
import traceback, json, os, logging

SCOPES = ["https://www.googleapis.com/auth/calendar.events"]
CLIENT_ID     = st.secrets["google"]["client_id"]
CLIENT_SECRET = st.secrets["google"]["client_secret"]
REDIRECT_URI  = st.secrets["google"]["redirect_uri"]    # must match console
...
def google_login() -> bool:
    ...
    # 2- Coming back from Google with ?code=
    if "code" in q:
        code  = q["code"][0]
        ...
        try:
            ...
            st.query_params.clear()          # wipe ?code from URL
            return True
...
def sync(creds,shifts,tz="Europe/Madrid"):
    ...
    existing=service.events().list(calendarId="primary",timeMin=now,
        privateExtendedProperty="shiftUploader=1").execute().get("items",[])
    by_key={e["extendedProperties"]["private"]["key"]:e for e in existing if "extendedProperties" in e}
    ins=upd=deln=0
    for s in shifts:
        body={
            "summary":f"P {s['start']}",
            "start":{"dateTime":f"{s['date']}T{s['start']}:00", "timeZone":tz},
            "end":  {"dateTime":f"{s['date']}T{s['end']}:00",   "timeZone":tz},
        ...
        if s["key"] in by_key:
            ...
            upd+=1; by_key.pop(s["key"])
        else:
            service.events().insert(calendarId="primary",body=body).execute(); ins+=1
    for ev in by_key.values():
        service.events().delete(calendarId="primary",eventId=ev["id"]).execute(); deln+=1
    return ins,upd,deln

# ---------- UI ----------
st.set_page_config("Shift Uploader","🗓️",layout="centered")
st.title("📤 Shift → Google Calendar")

if google_login():
    creds=creds_from_dict(st.session_state["creds"])
    pdf=st.file_uploader("Upload roster PDF",type="pdf")
    if pdf:
        with st.spinner("Parsing PDF..."):
            shifts=parse_pdf(pdf.read())
        if not shifts:
            st.error("Couldn’t find any shifts in that PDF.")
        else:
            st.write(f"Found **{len(shifts)}** shifts:")
            st.dataframe(pd.DataFrame(shifts),use_container_width=True)
            if st.button("Sync to Calendar"):
                with st.spinner("Syncing..."):
                    i,u,d=sync(creds,shifts)
                st.success(f"Inserted {i}, updated {u}, deleted {d}.")
else:
    st.info("Sign in to start.")
